...
class ConverterCalcUi(QMainWindow):
    ...
    def labelText(self, l_):
        duty, op_current, Ind_current, ip_current, i_ripple, Ind_cr, Ind, Ind_cr_ripple, max_indI, min_indI, cap, esr = l_
        ...
        txt_ = txt1 + txt2 + txt3 + txt4 + txt5 + txt6 + txt7 + txt8 + txt9 + txt10 + txt11 + txt12
        self.label = txt_
    ...
    def onIpEdit(self):
        try:
            ...
            self.errorLabel('Inappropriate block values. Block values must be of type Float.\nRetry!')
        finally:
    ...
    def evaluateResults(self):
        ...
        if Mode == 'boost':
            ...
            min_indcrnt = np.round(np.subtract(
                float(ind_current), float(I_ripple)/2), decimals=2)
            capacitor = boostlc.bst_cap_val(duty_, R, Vrp, fsw)
            Esr = boostlc.Esr(Vrp, Vo, max_indcrnt)
        elif Mode == 'buck':
            ...
            min_indcrnt = np.round(np.subtract(
                float(ind_current), float(I_ripple)/2), decimals=2)
            capacitor = bucklc.bck_cap_val(duty_, R, Vrp, fsw)
            Esr = bucklc.Esr(Vrp, Vo, max_indcrnt)
        elif Mode == 'buckboost':
            duty_ = bckbstlc.bckbst_duty_cycle(Vo, Vin)
            output_current = np.round(np.divide(Vo, R), decimals=2)
            ind_current = bckbstlc.bckbst_ind_current(
                duty_, output_current)
            input_current = np.multiply(ind_current, duty_)
            ...
            Esr = bckbstlc.Esr(Vrp, Vo, max_indcrnt)
        elif Mode == '--Select Type--':
            self.errorLabel('Converter type not selected!')

        params = [duty_, output_current, ind_current, input_current, I_ripple, ind_cr,
              ind_, ind_cr_ripple, max_indcrnt, min_indcrnt, capacitor, Esr]
