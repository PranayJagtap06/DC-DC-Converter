...
class Converter:
    ...
    def __init__(self):
        ...
        while True:
            try:
                ...
                self.Vrp = float(input("Enter % Output Voltage Ripple(mximum two decimal places): "))
                if self.Vrp > 1:
                    print(Fore.RED+"Ripple voltage must be greater than 1. Try again!\nRecommended 30%."+Style.RESET_ALL)
                    continue
    ...
    def run_converter(self):
        while True:
            ...
            if self.Vin > self.Vo:
                print(Fore.BLUE+"\n\tCritical Inductance(Lcr): {}H".format(format(self.buck_ind(), ".3e"))+Style.RESET_ALL)
                self.text_ind()
                print(Fore.BLUE+"\n\tCapacitor Rating(Co): {}F".format(format(self.buck_cap(), ".3e"))+Style.RESET_ALL)
                self.text_cap()
            else:
                print(Fore.BLUE+"\n\tCritical Inductance(Lcr): {}H".format(format(self.boost_ind(), ".3e"))+Style.RESET_ALL)
                self.text_ind()
                print(Fore.BLUE+"\n\tCapacitor Rating(Co): {}F".format(format(self.boost_cap(), ".3e"))+Style.RESET_ALL)
                self.text_cap()
    ...
    def text_ind(self):
        print(Fore.YELLOW+"\nCritical Inductance (Lcr) is the minimum value of the inductor for which the converter will operate in continuous current conduction mode."
            "Critical Inductance is the boundary between Continuous and Discontinuous conduction modes of the dc-dc converter. Below the critical inductance"
            "value of the inductor the converter will start performing in discontinuous current conduction mode. It is recommended to select inductor value"
            "slightly above the critical inductance value, '25%' higher the critical inductance is a decent choice."+Style.RESET_ALL)

    def text_cap(self):
        print(Fore.YELLOW+"\nThe Output Capacitor (Co) is used to keep the output voltage constant and the reduce the output Ripple Voltage. The higher the capacitor value the"
